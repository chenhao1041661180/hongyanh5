...
export default class AppTouch {
  ...
  _checkBeforeCreate(pageOBJ, name) {
    if (!pageOBJ || !name) {
      throw new Error('AppTouch实例化时，必须传入page对象和引用名')
      return false
    }
    if (pageOBJ[name]) {
      throw new Error('AppTouch实例化error： ' + name + ' 已经存在page中')
      return false
    }
    return true
  }
  ...
  start(evt) {
    ...
    if (this.preTapPosition.x !== null) {
      this.isDoubleTap = (this.delta > 0 && this.delta <= 250 && Math.abs(this.preTapPosition.x - this.x1) < 30 &&
  ...
  destroy() {
    ...
    this.preV = this.pinchStartLen = this.scale = this.isDoubleTap = this.delta = this.last = this.now = this.tapTimeout =
      this.singleTapTimeout = this.longTapTimeout = this.swipeTimeout = this.x1 = this.x2 = this.y1 = this.y2 = this.preTapPosition =
      this.rotate = this.touchStart = this.multipointStart = this.multipointEnd = this.pinch = this.swipe = this.tap =
      this.doubleTap = this.longTap = this.singleTap = this.pressMove = this.touchMove = this.touchEnd = this.touchCancel =
      null;
